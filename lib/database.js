...
async function initializeDatabase() {
  ...
  try {
    await client.query(`
      ...
      CREATE TABLE IF NOT EXISTS wiki_page_gallery (
        id SERIAL PRIMARY KEY,
...
const db = {
  ...
  // Wiki submissions
  createWikiSubmission: async (pageId, slug, title, content, createdBy, featuredImageUrl = null, galleryImages = []) => {
    const client = await pool.connect();
    try {
      await client.query(
        'INSERT INTO wiki_submissions (page_id, slug, title, content, created_by, featured_image_url, gallery_images) VALUES ($1, $2, $3, $4, $5, $6, $7)',
        [pageId, slug, title, content, createdBy, featuredImageUrl, JSON.stringify(galleryImages)]
      );
    } finally {
      client.release();
    }
  ...
  approveWikiSubmission: async (submissionId, reviewerId, reviewerNote = null) => {
    ...
    try {
      ...
      if (submission.page_id) {
        await client.query(
          "UPDATE wiki_pages SET title = $1, content = $2, updated_at = CURRENT_TIMESTAMP, status = 'approved', approved_by = $3, featured_image_url = $4 WHERE id = $5",
          [submission.title, submission.content, reviewerId, submission.featured_image_url || null, submission.page_id]
        );
        pageId = submission.page_id;
      } else {
        const pageResult = await client.query(
          "INSERT INTO wiki_pages (slug, title, content, created_by, status, approved_by, featured_image_url) VALUES ($1, $2, $3, $4, 'approved', $5, $6) RETURNING id",
          [submission.slug, submission.title, submission.content, submission.created_by, reviewerId, submission.featured_image_url || null]
        );
        pageId = pageResult.rows[0].id;
      }

      // Save gallery images if present
      if (submission.gallery_images && typeof submission.gallery_images === 'string') {
        const galleryImages = JSON.parse(submission.gallery_images);
        for (const image of galleryImages) {
          await client.query(
            'INSERT INTO wiki_page_gallery (page_id, image_url, caption) VALUES ($1, $2, $3)',
            [pageId, image.url, image.caption || null]
          );
        }
      }
  ...
  // Wiki page gallery
  addWikiPageGalleryImage: async (pageId, imageUrl, caption = null) => {
    const client = await pool.connect();
    try {
      const result = await client.query(
        'INSERT INTO wiki_page_gallery (page_id, image_url, caption) VALUES ($1, $2, $3) RETURNING id',
  ...
  getWikiPageGallery: async (pageId) => {
    const client = await pool.connect();
    try {
      const result = await client.query(
        'SELECT * FROM wiki_page_gallery WHERE page_id = $1 ORDER BY created_at ASC',
        [pageId]
      );
      return result.rows;
    } finally {
  ...
  deleteWikiGalleryImage: async (imageId) => {
    const client = await pool.connect();
    try {
      await client.query('DELETE FROM wiki_page_gallery WHERE id = $1', [imageId]);
    } finally {
      client.release();
    }
  },

  updateWikiPageFeaturedImage: async (pageId, imageUrl) => {
    const client = await pool.connect();
    try {
      await client.query(
        'UPDATE wiki_pages SET featured_image_url = $1 WHERE id = $2',
        [imageUrl, pageId]
      );
    } finally {
      client.release();
